...
def payload(**kwargs):
    data = dict(balance_version=1, crop='wheat', soil_type='loam', area=1,
                area_unit='hectare', irrigation_type='drip', day_of_growth=60,
                yesterday_deficit=0, field_type='open', is_saline='no')
    data.update(kwargs)
    return data
...
class BalanceTests(unittest.TestCase):
    def test_reference_balance_and_cost(self):
        field = parse_field(payload(yesterday_deficit=30, area='6,7', power_price=25, energy_kwh_m3=.2))
        ...
        self.assertAlmostEqual(r['deficit'], 29.75)
        self.assertAlmostEqual(r['gross_m3'], 29.75 * 10 * 6.7 / .9)
    ...
    def test_all_method_thresholds_and_raw_priority(self):
        for method, (threshold, eta) in METHODS.items():
            for d, status in [(threshold-.01,'deferred'),(threshold,'irrigate'),(threshold+.01,'irrigate')]:
                r = calculate_balance(parse_field(payload(irrigation_type=method, yesterday_deficit=d)),0,0)
                self.assertEqual(r['status'], status)
                self.assertAlmostEqual(r['gross_m3'], 0 if status == 'deferred' else d*10/eta)
        f = parse_field(payload(soil_type='sand',day_of_growth=0,irrigation_type='furrow',yesterday_deficit=10))
        self.assertEqual(calculate_balance(f,0,0)['status'], 'critical')
        raw = calculate_balance(f,0,0)['raw']
        f = parse_field(payload(soil_type='sand',day_of_growth=0,irrigation_type='furrow',yesterday_deficit=raw))
        self.assertEqual(calculate_balance(f,0,0)['status'], 'deferred')  # Strict > RAW per project policy.

    def test_rain_cutoff_clamping_and_repeatability(self):
        f = parse_field(payload(yesterday_deficit=10))
        ...
        r = calculate_balance(parse_field(payload(yesterday_deficit=1000)),0,0)
    ...
    def test_bad_inputs_and_missing_fields(self):
        for key in ['soil_type','day_of_growth','yesterday_deficit','balance_version']:
            d=payload(); del d[key]
            with self.assertRaises(BalanceInputError): parse_field(d)
        for value in [None,True,'NaN','inf','-1','6,7,8','']:
            with self.assertRaises(BalanceInputError): parse_field(payload(yesterday_deficit=value))
    ...
    def test_units_greenhouse_saline_custom_rice(self):
        a=calculate_balance(parse_field(payload(area=1,yesterday_deficit=10)),5,0)
        b=calculate_balance(parse_field(payload(area=100,area_unit='sotka',yesterday_deficit=10)),5,0)
        ...
        sal=calculate_balance(parse_field(payload(area=1,yesterday_deficit=10,is_saline='yes')),5,0)
    ...
    def test_reports_both_languages_all_statuses(self):
        ...
        for lang in ['ru','kz']:
            for d in [0,20,120]:
                f=parse_field(payload(yesterday_deficit=d))
